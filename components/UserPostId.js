...
export default function Post({post,id}) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      collection(db, "Userposts", id, "likes"), (snapshot) => setLikes(snapshot.docs)
    );
  }, [db])

  useEffect(() => {
    const unsubscribe = onSnapshot(
      collection(db, "Userposts", id, "comments"), (snapshot) => setComments(snapshot.docs)
    );
  }, [db])
  ...
  async function likePost(){
    if(currentUser){
      ...
      if(hasLiked){
        await deleteDoc(doc(db, "Userposts", id, "likes", currentUser?.uid ))
      } else {
      await setDoc(doc(db, "Userposts", id, "likes", currentUser?.uid ), {
  ...
  async function deletePost(){
    if(window.confirm("Are you sure you want to delete this post?")){
      ...
      deleteDoc(doc(db, "Userposts", id))
      if(post.data().image){

        deleteObject(ref(storage, `Userposts/${id}/image`))
      }
      router.push("/")
    } 
  }
